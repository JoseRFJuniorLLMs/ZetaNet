...
# ### ADIÇÃO: Importar pickle para salvar/carregar scalers
import pickle
    ...
    def train(self, train_loader, val_loader, epochs=200, learning_rate=0.001, patience=20):
        ...
        for epoch in range(epochs):
            ...
            if val_loss < best_val_loss:
                ...
                patience_counter = 0
                # Salvar melhor modelo (já estava aqui na sua versão)
                torch.save(self.model.state_dict(), 'best_zetanet.pth')
                print("   -> Melhor modelo salvo!")  # ### ADIÇÃO: Mensagem de confirmação
            else:
                patience_counter += 1
        ...
        # Carregar melhor modelo (já estava aqui na sua versão)
...
def main():
    ...
    FILEPATH = r"/content/combined_zeta_data.csv"  # Usando 'r' para raw string
    ...
    # ### ADIÇÃO: Salvar os scalers (necessário para inferência posterior em outro script)
    try:
    ...
    # Criar data loaders
    train_loader, val_loader = create_data_loaders(data_dict,
                                                   batch_size=128)  # Mudei o batch_size para 128 (padrão anterior)
    ...
    model = ImprovedZetaNet(
        ...
        output_size=2,
        dropout_rate=0.1
    ).to(DEVICE)  # ### ADIÇÃO: Garantir que o modelo vá para o dispositivo
    ...
    # Plotar resultados (já inclui avaliação no conjunto de validação)
    plot_results(trainer, data_dict, model)

    # ### ADIÇÃO: Exemplo de Inferência (Previsão em um novo dado)
    print("\n--- Exemplo de Inferência (Previsão em um novo dado) ---")
    # Para inferência, você carregaria o modelo e os scalers.
    # Exemplo de carregamento em um script separado (ou aqui para demonstração):

    try:
        # 1. Carregar o modelo treinado
        loaded_model = ImprovedZetaNet(
            input_size=2,
            hidden_sizes=[128, 256, 256, 128, 64],
            output_size=2,
            dropout_rate=0.1
        ).to(DEVICE)
        loaded_model.load_state_dict(torch.load('best_zetanet.pth'))
        loaded_model.eval()  # Definir para modo de avaliação
        ...
        # 3. Definir um novo ponto de dados para previsão (ex: um valor 's' qualquer)
        new_s_real = 0.5
        new_s_imag = 100.0  # Um ponto de exemplo

        # 4. Pré-processar a entrada usando o scaler_X carregado
        new_data_unscaled = np.array([[new_s_real, new_s_imag]])
        new_data_scaled = loaded_scaler_X.transform(new_data_unscaled)
        new_data_tensor = torch.tensor(new_data_scaled, dtype=torch.float32).to(DEVICE)

        # 5. Fazer a previsão
        with torch.no_grad():
            prediction_scaled = loaded_model(new_data_tensor)

        # 6. Inverter a normalização da previsão usando o scaler_y carregado
        prediction_unscaled = loaded_scaler_y.inverse_transform(prediction_scaled.cpu().numpy())
        ...
        print(
            f"Erro ao carregar modelo ou scalers para inferência: {e}. Certifique-se de que 'best_zetanet.pth', 'scaler_X.pkl' e 'scaler_y.pkl' existem.")
    except Exception as e:
